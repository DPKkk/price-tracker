...
const callApi = async (productName, websiteNames) => {
    const data = {
      ...
      messages: [
        {
          role: "user",
          content: `Give me a JSON array of ${productName} listings in India from ${websiteNames.join(', ')}. For each, include: site, productUrl, productName, price, rating, and imageUrls. Format response as raw JSON only, no extra text.`,
        },
      ],
    };
